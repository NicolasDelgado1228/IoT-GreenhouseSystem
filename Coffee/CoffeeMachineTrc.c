/*******************************************************************************
*
*  CoffeeMachineTrc.c -   A simple trace-enabled program implementing the use
*                         case MakeCoffee for the Coffee Machine model v1
*
...
#include    <string.h>
#include    <errno.h>
#include    "phtrdsMsgLyr.h"              /* pthreads message layer fucntion
...
static void *pController ( void *arg )
{
  ...
  for ( ; ; )
  {
    ...
    printf ( "\tController received signal %d, value %d in state %d\n", InMsg.signal, InMsg.value, state );
    fflush ( stdout );
    ...
    /* show next state */
    printf ( "\tController next state is %d\n", state_next );
    fflush ( stdout );
  }
